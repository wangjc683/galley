...
**三条铁律**
1. Master 只做两件事：**拆**（把阶段目标切成互不重叠的独立子任务派给 worker）和**汇**（汇总产物、判断、排序、把验收通过的产物折叠进锚点交付物）。绝不认领或持有 worker 的 task（task 归 worker，靠 claim 认领；创建 task 时绝不加 `--owner-session`），也绝不自己下场做某个单点子任务——但"汇"（synthesis、维护锚点交付物）就是你的活，别把它误当成"生产产物"而回避。
