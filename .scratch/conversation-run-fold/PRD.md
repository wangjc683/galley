...
- **修订（2026-09-16）：完成即折。** JC 真机看十步 run 完成后全展的
  画面「太占空间」，而两轮挤压像素（步间 8px、pill 上 summary 行）都以
  阅读体验变差告终，裁定空间问题按状态分流而非挤压：过程区的层级靠
  折叠，不靠密度。折叠头此时已有步数 / 用时 / 工具计数摘要，「回看刚才
  做了什么」一击可达。「立刻折叠会抽走视口内容」的顾虑用动画化解：
  keep-expanded 指针在 run 落定后**下一帧**释放，RunFoldSection 以打开态
  挂载再走 grid-rows 收合，不是瞬间消失。视口滚动位置在收合中的表现待
  真机验（devlog 后记九）。
- **再修订（同日）：live 两行窗口。** 见 [live-run-window PRD](../live-run-window/PRD.md)。
  live 期间只展示最后一个落定的步 + 进行中行，之前的步折进「已完成 N 步」
  live 头（默认折、点开 opt-in、完成时尊重 override）。完成即折因此只收走
  两行，上一条「下一帧释放指针」的 rAF 机制随之删除：settled 折叠状态直接
  由 `override ?? true` 得到。
...
进入 live → 指针转移 → 旧 run 折叠~~ 2026-09-16 起：run 完成后一帧
（rAF）释放指针 → 折叠头收合动画。
...
**2026-09-23 修订（JC 真机切换器裁决，[devlog](../../docs/devlog/2026-09-23-run-fold-header-hierarchy.md)）**：
行内分层——数字 ink-soft、其余 muted，气味段降到 pill 标签字号，竖线两侧
拉开；气味段改文件的工具排最前（修订上文「按次数降序」的截断政策：改文件
之外的部分仍降序）。
