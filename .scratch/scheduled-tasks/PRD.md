...
### 14. 可靠性前提引导（2026-07-30；见 issues/14）

dialog footer 升级为条件式：自启未开且有启用任务时显示警示 + 行内
一键「设为开机自动启动」，解决后自动复原。情境提示而非 Settings
搬迁；不弹模态、不静默改登录项、不纠缠「关窗驻留后台」（用户显式
选择过且默认开）。
