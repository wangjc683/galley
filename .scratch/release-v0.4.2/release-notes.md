...
- Each step header stays a header: when a model skips its own one-line recap, the step line shows a short note instead of repeating the whole answer that follows it.
...
- 步骤标题回归标题:当模型没有写自己的单行小结时,「第 N 步」那一行会显示一句简短说明,而不是把下方整段回答重复一遍。
