...
📎 增加「添加文件…」入口（文案见定案 10），经 `plugin-dialog` 原生
选择器（已在依赖中，返回真实路径）走同一段占位符插入逻辑。服务不爱
拖拽 / 全键盘流用户，边际成本低。与拖放同期交付（v1）。
...
footer hint（空稿空闲时显示「拖入任意文件或文件夹，发给 AI」，打字
...
### 10. 文案面向普通用户，「引用」词汇全面清除（2026-07-29 JC 裁决）

「引用路径」是把实现机制写进 UI。全部用户可见文案改为结果导向：hint
「拖入任意文件或文件夹，发给 AI」、overlay「拖放以添加图片或文件」、
📎 菜单「添加文件…」、tooltip「添加图片或文件」。要点：

- 收件方用「AI」不用「Galley」/「模型」：copy 规范 Agent vs AI 一节已
  裁决对话对象称 AI（先例「已通知 AI」）；且「发给 Galley」在机制上是
  一件没发生的事——文件不入库、不导入，AI 去原地读，误导用户以为文件
  已存进 Galley。「发给 AI」描述的才是真实结果。
- overlay / 菜单用「添加」不用「发送」：拖放只是加进草稿，发送在其后。
- 「引用 vs 附件」的区分保留在 PRD / issue / 代码注释层；UI 上由
  `[File #N: xxx]` 令牌自解释。i18n key 名不变（key 是开发者语汇）。
- 经「添加文件…」选中的图片仍作路径令牌（issue 04 行为不变），令牌
  立刻可见、行为自解释，暂不改道附件管线。
