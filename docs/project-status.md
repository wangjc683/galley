...
- Git tag / GitHub Release: `v0.3.3` is the current published stable release.
- Agent API schema: `schemaVersion: 1`
- Release tier: stable patch; default update channel points at `v0.3.3`.
...
`v0.3.3` is a desktop usability release. Galley now remembers window state,
uses more native-feeling desktop interactions and Windows scrollbars, keeps
conversation loading stable, and reports real updater progress. Model setup in
Onboarding and Settings requires fewer manual steps, while the new General tab
adds launch-at-login with silent background startup. Older sessions are easier
to recover when the recent window is empty, and the composer no longer repeats
`/btw` guidance during an active turn. Product shape, the Bundled GA baseline,
Agent API schema (`schemaVersion: 1`), and update-channel policy stay unchanged.

## Current Release State

`v0.3.3` is published and promoted as the live stable release. The default
`updates/stable/latest.json` channel points at `v0.3.3`, with the legacy
...
| Release path | v0.3.3 stable patch is published and promoted on the stable update channel | [release / update SOP](./release-update-sop.md) |
...
- Current package metadata uses `0.3.3`. For the next release, bump every
