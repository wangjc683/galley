...
### 2026-09-23
- [v0.5.3 发布](./2026-09-23-v0.5.3-release.md) — v0.5.2 后一天：思考实时预览（`0016` 重写，打包门禁 mac-x64 过）+ 对话区四项打磨（回声步标题、步骤行 hover、中文加粗、折叠头分层）；patch；v0.5.1 → v0.5.2 应用内更新 JC 已验核销；推送第一次撞上 GitHub 按 tag 查 release 的旧数据（资产为空），推送流程改为按 release id 下载后推送成功；漏提 GA baseline 审计（上游多两个与 Galley 无关的提交），已补进 SOP pre-flight
- [折叠头行内分层：数字提墨、气味段降档、改文件排最前](./2026-09-23-run-fold-header-hierarchy.md) — JC 嫌折叠头整行一个层级；先对齐 09-16（整段加深加粗被否）与 08-06（「用时」防误读、整行 muted）；数据：步数 / 用时真在变，气味段 54% 是三种签名、改文件 11% 常落队尾；JC 自述只看时长、扫步数；层级 v0/v1/v2 × 气味 full/files/signals 切换器真机，我推荐 v1 且判 files 最弱，JC 定 files + v2（只数字 ink-soft，气味段降到 pill 标签字号）；v2 与 09-16 被否方案的区别记清防误判；改文件提墨不做
- [中文加粗漏星号：换 remark-cjk-friendly，删自写插件](./2026-09-23-cjk-friendly-emphasis.md) — JC 在揭阳 session 最终回答里看到满屏 `**`：模型写 `**标签：**正文`，CommonMark flanking 规则下紧贴全角标点的结束符不成立；06-09 自写插件只修引号一种形态；全库 751 条重渲，64 个漏星号 → 2（剩下是模型写坏的 `** Spurs`），零回归、自写插件被完全覆盖故删；全入口一起变（含本地 md，比 GitHub 宽容，接受）；删除线配套包不加
...
### 2026-08-31
- [v0.4.11 发布](./2026-08-31-v0.4.11-release.md) — 单 headline 小版本：无鉴权 Provider（galley#24）+ GA baseline `30b24ad→efb3bc6` 顺带；触发器复用 v0.4.8/9/10 的「用户上报修复压着不发会加厚下次 delta」，反方（占位 key workaround 在手、紧迫度低）摆过被压过；patch 定级无争议；打包门禁 mandatory（runner+managed-ga 都动了）mac-x64 从零绿、其余平台交 release.yml tag 时守门；draft smoke 一次覆盖三件：no-auth 流程、新 baseline 多步会话（abort 响应性应变好）、SOP 第 8 步 dogfood 债
- [GA 上游升级 30b24ad -> efb3bc6](./2026-08-31-ga-upstream-upgrade-30b24ad-to-efb3bc6.md) — v0.4.11 预备，兑现 v0.4.10 留下的「下次发版再审」并把当时的 10 提交一并收编；19 提交 168 文件 ~17.1k 行但大头全是上游 Desktop 2.0（src-tauri/desktop_bridge/发布资格工具/测试），引擎核心只有 `llmcore.py`（+28/−8）与 `agentmain.py`（+11）且**全是 Galley 正收益**：可中断重试退避（0.2s 轮询 should_stop，退避窗口里点停不再干等）+ abort 强制唤醒阻塞 recv（socket `_real_close`，上游 Windows 实测）+ 裁剪线性化（语义与 history 形状不变）；补丁栈 rebase 一处真冲突——**0017 × 上游逐调用 token ledger** 撞行，合成保两边（ledger 在 Galley 路径构造性惰性：`init_ledger` 只有上游 desktop bridge 调）；预判会炸的 0007（codex 插 152 行撞同函数重写）反而三方合并干净；**打包门禁修自家断言不修上游**——烟测从立门起就 import 旧社区版 `desktop_bridge` 当 aiohttp 代理，Desktop 2.0 改脚本式兄弟导入后包式 import 必炸，降级 `find_spec` 存在检查，教训「烟测别执行不属于自己执行路径的上游模块」；另一条验证纪律：门禁失败曾被 `cmd | tail` 吃掉退出码，跑门禁别尾接管道；兼容矩阵 235 passed + bundle 从零重建 161M + drift gate 四面同步 OK
...
### 2026-08-23
- [多步 run 过程区密度 pass](./2026-08-23-step-density-pass.md) — JC 两问：多步 run 过程区空间利用率偏低、层级是否应低于 final answer；算账后一步 86px 留白占 ~55%、13 步超一屏，诊断三因（章节级 `mt-6` 用在两根细线的日志级内容上——间距演化史第 5 条的 24px 裁决隐含「章节有正文」前提；一个事件拆两行、裸步只剩纯数字孤行；宽栏两端对齐 pill 的横向死区），关键论证是**纵向体量本身是层级信号、压缩即降级**——「提密度」与「降层级」是同一刀；反方（settled run 会折叠、曝光有限）也摆了但 live 观看是每个多步 run 的必经曝光；裁决 A 最小刀落地：run 内两档间距（边界 `mt-6` / run 内 `mt-3`，判据 `index === 1` 因 GA 每 `put_task` 步号重数、免穿 run-group，顺带保住 RunFoldSection 的 `-mt-2.5` 折叠数学）+ 裸步合并（无 summary/detail/narration 且恰一个 settled inline 工具 → marker 行取消、pill 自渲染「第 N 步 │」前缀，前缀在 button 外保左列对齐）；两行步 −28%、裸步 −56%；B 全面单行合并 / C rail 重塑 / 横向案暂缓进 deferred；数值不开变体直接落 12px（收益大头在结构不在手感票）；`conversation.md` 间距史第 6 条带第二层教训：**间距裁决要连同内容前提一起记**
- [Goal 派发装门与真实忙闲信号](./2026-08-23-goal-dispatch-gate-and-run-state.md) — 用户群报 solo Goal 两症状：galley#19 错误 toast 刷屏（任务却在推进）+ 停止后 sidebar 秒变「已停止」实际还在干活；根因是**液位计失灵不是竞态**——goal 控制器判忙闲读 SQLite `sessions.status` 而 Core 从不写 `running`（transient status 只活在内存），所有等待门永真，`wait_solo_turn` 退化成「出现新消息行」而消息行每步一行，nudge 每步补发一条、又因 `session.goal_solo_turn`/`goal_synthesize`/`goal_master_plan` 绕过出站队列直发 bridge 而每条撞防御门变 toast；停止收尾同理误把旧 run 的中间步骤行锚成收尾答案提前写 `Stopped`；修复三层：内部 socket 读命令 `session.run_state`（additive）暴露 `openRun`（只在 RunComplete 关，跨轮间空隙）等真信号、`try_reserve_run` 原子占门 busy 即零副作用拒发（CLI 等待后重新生成 nudge 而非排队旧文案）、finish 派发 busy 重试且每次尝试前重取 baseline 让误锚机制性消失；已否：nudge 进队列（过期文案 + 脚手架文本会进 GUI 排队 UI）、动 sidebar（显示忠实、病在状态源头）、拿 `agent_running` 当等待信号（轮间闪断）；「停止立即 abort 当前轮」记 deferred

### 2026-08-21
- [GA 上游升级 f06d550 -> 30b24ad](./2026-08-21-ga-upstream-upgrade-f06d550-to-30b24ad.md) — 8 提交 7 文件（~52/~31），baseline 引入以来**最小的一段**（从上一段手里接过名号），也是 baseline 有记录以来**第一次零冲突 rebase**（此前每段至少一处真冲突）；引擎核心只动 `ga.py` 1 行 + `llmcore.py` 5/2，`agent_loop.py`/`agentmain.py` 零 diff，`pyproject.toml` 只动 `ui` extra 的 `streamlit>=1.28→>=1.62` 而 **`[project.dependencies]` 未变**（`GA_DEPS` 镜像的是核心清单、streamlit 从不在内），但**仍按 08-14 的教训从零重跑打包门禁**没拿「GA_DEPS 未变」去省；三条引擎改动里**只有一条需要 dogfood 盯**——上游 `3f39e2b` 把 `payload["context_management"]`（`clear_thinking_20251015`）注释掉而 `anthropic-beta` 头里的 beta 留着、commit message 未给理由，Galley 耦合面为空（`0016`/`0017` 读的是 SSE 流不是请求体）故零改动，但 native Claude 长会话的 input token 计数可能变；`api_key_header`（社区 PR #751，`auto`/`x-api-key`/`bearer`，为「说 Anthropic 协议但 key 非 sk-ant- 前缀」的中转而生）是**上游新能力 Galley 免改即通**——`managed_model_config_from_env` 的 `cfg.update(advanced)` 让 `advancedOptions` 原样透传，当初把它做成自由字典而非白名单的红利，差的只是 GUI 入口（高级面板是策展字段集）已记台账；`ga.py` 那 1 行 `'!!!Error:' in content[-100:]→content[50:][-100:]` **是一致性修复不是策略反转**——只影响短内容，长的 `!!!Error: HTTP 4xx: {body}` 因标记在头部**改之前就已落不进判定**，改后短硬失败第一次就交出错误文本而不先烧三次重生成，探针不受影响（`probe.rs` 直调 `raw_ask` 不进 `do_no_tool`）；**零漂移预期下仍走 rebase 脚本的理由值得留痕**——预期只有 +3 行纯位置漂移（`llmcore.py` 762 附近），听着正是「手改两个行号就行」的量级，但 `0002` 是 `@@ -1011,0 +1012,47 @@` **零上下文纯插入**：没有被删除行供 `git apply` 校验，错位不报错只静默落错地方，正是 07-15 踩过、`py_compile` 扫描要防的那一类；结果三个补丁（`0001`/`0002`/`0008`）**只有行号变、正文一字未改**；验证：19/19 clean + py_compile OK + payload 零差异 + drift gate OK + 234 passed + 打包门禁从零重建 161M import OK
- [用户消息质感实验（零采纳）](./2026-08-21-user-message-texture-experiment.md) — JC 满意 sidebar 选中行后回看主区：「用户消息有点死板和太硬，是否也学 sidebar 变成圆角的、带抬升质感的」；先自查排除嫌疑（**不是刚才两轮改出来的**：brand-tint 相对主区地面的 Δ 在 08-06/抬画布/翻转三阶段是 18.63→18.79→**18.88**，漂移 0.25 不可察觉，light 更是从未动过，「硬」是形态一直如此、只是刚看完 sidebar 抬升产生了对比）；**「抬升」被否且理由不是「08-06 说过不行」**（那次根本没讨论过投影）**而是寄存器冲突**——用同期刚确立的 chrome/content 二分：sidebar 选中行的抬升成立正因为它是 chrome（控件语汇、卡片浮在材质上），用户消息在 content 层是**纸上的墨**，高亮笔痕迹渗进纸里不浮在纸面，加投影等于宣称「这块颜色悬在纸上方」、隐喻自相矛盾，而这恰是 08-06 花两轮 A/B 治的**寄存器倒挂**（机器圆润、人方正）换个方向再犯，且 08-10 的 copy chip 已经实犯过一次并修回（floating 装甲紧贴笔触 → 改裸 inline）；**这条论证是本篇最该留的东西**，以后再有「让用户消息更立体/更有卡片感」的提案从这一行开始谈；随后重述问题为「纸墨语汇里质感来自墨的物理特性不来自物体悬浮，当前这支笔画得太完美——纯色平涂、边界绝对整齐，那不是笔痕是印刷色块」，搭两维度切换器实测：**墨水沉积**（每道笔触下缘 6px 软渐变压深，light 压暗/dark 提亮即方向随主题翻转，档位 3/4/6 L\* 对应 ΔE 0.026/0.035/0.051，**先算 ΔE 再提案**避免重演同日「色相归一」那种改了也看不见；它同时是 08-06「b-gap 留缝 vs b-fused 融合」之外的**第三条路**——`box-decoration-clone` 逐行重绘背景故沉积边能让融合块重新浮现行节奏而不必把缝放回来）× **笔触圆角**（2/4/6px，上限取 `--radius-sm`，再大进 chip 语汇等于把 08-06 否掉的 chat bubble 从后门放回来）；**JC 实测「效果都一般，先不改」，零采纳、脚手架全拆、零代码改动**；讨论中另**验证否掉四个候选留痕防重提**：半透明墨水（听着最忠于隐喻但**纯色底上半透明与不透明渲染结果完全一致**，除非先加纸纹）、斜切笔头（隐喻满分但 `clip-path` 会裁掉画出头用的偏移 box-shadow，要重写那个已踩过 WKWebView 折行坑的方案）、SVG 扰动边缘（花哨，冲 paper-quiet）、内阴影（方向上与「渗进纸里」兼容但 `shadow-inner` 已被控件占用会造成寄存器混淆）；**顺带发现一笔欠账并记台账**：`--color-brand-tint` 的 dark 值一直标着「provisional，等 dark pass 复核」而 08-21 的抬画布+翻转**就是那个 dark pass**，我只让它跟着阶梯移动没复核响度——数据可疑（dark Δ 18.88 vs light 8.39，**响 2.25 倍**，且三阶段稳定说明是 08-06 定值时就带着的、从未单独审过），本次未处理是因为 JC 明确说看的是 light 而 light 响度正常
- [Sidebar 选中行三通道](./2026-08-21-sidebar-selected-row-three-channels.md) — JC「选中的质感偏弱、不够一眼扫到」，`deferred` 里 08-07 那条的启动信号（「dogfood 中觉得选中行不够醒目——出现即证据」）**原样命中**，且同条目还记着当天 JC 真机确认「现状 OK」，**十四天后改口本身就是数据**；诊断的关键动作是**先数通道再调数值**——把行上所有视觉通道列表后看得见：底色被 selected/hover/warning/error/actionsOpen/editing **六个状态共用**，而 running 独占四条（呼吸 rail + 图标 + semibold + brand subline），于是「你在这里」这个最基础的导航锚点表达最单薄、且与 hover **同类物质只是浓度不同**，所以「质感偏弱」的准确说法是**没有属于自己的通道**，在拥挤通道里继续加响度只会读成「更用力的 hover」；light 那一半是**真 bug**：selected 相对 chrome ΔL\* **+0.01**（完全为零，全靠杏色相撑）而 hover 反向 −3.81，**划过一行比选中一行更显眼**；JC 的分主题实感精确对应两层病因（light 明度 bug / dark 通道单一），搭三维度变体切换器（填充 × 质感 × 淡化）真机裁决，搭之前先算清一条**不对称约束**否则测试白做——light 往亮只有 ~2.6 点空间（chrome 已在 L\* 94.12、杏色再亮就褪白）、往暗有 ~9 点且能同时提彩度；裁决为**压深 + 抬升 + 开淡化**，「形状」（贴满栏宽 + 直角）落选；落地后选中态占三通道：底色（`#e7d8c0`，ΔL\* −7.17 vs chrome / −3.36 越过 hover，彩度 ×1.3 保住杏色）+ 抬升（新 `--shadow-selected`，全行唯一投影）+ 减法聚焦（选中行标题保持 ink、其余降 ink-soft）；**有意不用左 rail**（属 running/waiting/error，而**选中行常常同时就是 running 行**，这条硬约束直接让「左侧实心条」这个最经典解法变贵）**和标题字重**（属 running/unread）；填充走 `.chrome-hover-scope` 覆写不动全局——`bg-selected` 全仓 8 处在 sidebar 之外坐在 app/elevated 上，而 QuickActions / ProjectReview 的同名毛病被顺带治好，正是 08-07 选 scope 而非新 token 时列的好处；**顺带撞见一个既有 bug 未修已记台账**：Tailwind v4 给 `@theme` 的 `--shadow-*` 生成 utility 时**内联值不生成 `var()`**（`.shadow-card{--tw-shadow:0 1px 2px …#1f1b170a}` 写死 light 的 4% 暖黑），故 dark 块那批 `--shadow-*` 重定义**对直写调用点完全不生效**——实测 **52 处直写受影响 / 36 处 var 写法幸免**，后者分布（button、composer、MessageUser）本身是线索：07-16 那轮显然踩过但没留字；本轮只保证自己不踩（写 `shadow-[var(--shadow-selected)]`），全量修复要连带复核 dark 阴影（改完会**第一次真正生效**、观感必然变重，当年很可能就是照着「看不见」调的值）；**同日续裁「能否与用户消息带统一」**——JC 观察两色很接近想合并，量化后 ΔE **0.026**（正落在「勉强可察觉」档：不够远到读作两个东西、不够近到读作一个东西，最膈应的量级，直觉准得很精确）；**这一算顺带否掉了我自己差点提的方案**：light 杏沙家族色相散得离谱（实色锚 H 51、三个淡填充散在 62.6–81.5 跨 19 度）看着显然该收敛，但归一的改变量只有 ΔE 0.0026–0.010——**C≈0.03 的淡色上色相维度基本是废的**，教训是「色相盘点看着离谱 ≠ 视觉上有问题，先算 ΔE 再提方案」；**light 不能统一、dark 能，且这是 chrome 翻转的直接推论**——dark 的 chrome 现在比 app 亮而填充往亮走，按 app 标定的 brand-tint 绝对值更高、落到 chrome 上仍高过按 chrome 标定的 selected（+16.42/+8.57，还把 selected→line 从 +3.31 拉到 +6.22），light 每个符号都相反、同一个值掉到离 hover 仅 **−0.73** 处即与 hover 不可分；裁决的关键动作是**把「统一」这个词捆着的两件事拆开**——① 视觉后果 **≠ 呼应感**（两色从不在同一视线焦点并排出现：隔着全高 separator、形态一个是 48px 整行填充一个是行内 box-decoration-clone 文字带，故「同色」在使用中感知不到），真实可感知的只有「dark 选中行响 22% 且离 hover 更远」，采纳；② 概念后果才是统一的真正内容，**不绑定**——语义不同（「这段话出自你」重复多次 vs 「你在这里」全局唯一）须能独立演化，写成 `var(--color-brand-tint)` 会让日后调主区消息带**意外改掉 sidebar 选中行**（正是 08-05→08-07 hover 失明那类共用 token 机制），且 light 不绑/dark 绑会让**规则本身**跨主题不一致、与刚定的「规则统一、数值随主题」相悖；落地为 dark scope `--color-selected` `#44352a→#4d3b2b` 独立标定值 + 注释写死「同值是标定重合不是绑定」
- [Dark 抬画布 + chroma 松到 0.73× + chrome 翻向](./2026-08-21-dark-canvas-lift-and-chroma-reloosen.md) — JC 提案「dark 下 sidebar 和 mainview 背景对调」，**参考主题证伪了他自己的归因、同时交出了真因**：Tokyo Night（JC 日常主题）的 sidebar 也比编辑区暗（nvim `bg_dark #16161e` vs `bg #1a1b26`；VS Code 版 `sideBar`/`activityBar`/`panel` 全 `#16161e` vs `editor #1a1b26`），方向从来不是问题；把两者放进同一坐标系后真因是三条——① 绝对深度：TN **最暗**的 UI 面（L 20.4）比 Galley 的**整个对话画布**（19.3）还亮，我们的 sidebar 在 15.6 贴地板 ② 差值 3.7 是 TN 的 2.2 的 1.7 倍 ③ 地板处 C 近零，**近黑 + 近零饱和的大色块读作「重物」而非「后退」**，这才是「头重脚轻」的物理成因；于是兑现挂了近一个月的 deferred「抬画布」（启动信号「整体太沉太黑」以「sidebar 太重」的外衣到达，且 08-05 的 chrome 加深把地板问题又恶化了 1.9 点）：整条阶梯 +3.3 OKLCH L、chrome +4.7 把差值收到 TN 的 2.2（ΔL\* 3.04→2.52）；**chroma 是对 07-25「0.6×」的有意部分重开**——只抬 L 会让 C/L 从 .0264 悄悄掉到 .0224（等于又降一档），故先按 L 比例补偿回 0.6× 再松 1.21× 至 ~0.73×，JC 在三档 hex 阶梯预览中选定；仍远低于 TN 的 .078–.095 **且刻意如此**（差的是色相不是幅度，暖色在冷蓝紫还撑得住的 chroma 上就发闷），另一半靠抬亮本身治 muddy；沿用 07-25 两条既有规则：墨色同取 1.21× chroma 但**明度不动**（对比度漂移 ≤0.03，17 处 shadow 高光随 ink 更新、处数与当年记录吻合是一次交叉验证）、品牌填充只抬 L 不吃档位（它们当初就没参与降暖）；**正文对比有意留在 13.96:1**（TN 是 10.59），要往那个区间走必须压墨色明度、会重开 07-25 暖轴裁决，JC 裁决独立一轮；否决的原提案「对调」除被 TN 证伪外还有硬伤——会把长文阅读面推到地板（对比反升到 15.9，与「读久了累」相反），且 `bg-app` 不只是画布还是卡片内部凹陷填充（`MessageAgent` 工具输出块等）会一起沉；「对调 + 抬画布」作为**未来可选路径保留**（JC 已明确能接受「一列浮起来的材质」，届时不必再抬）；复核项：`chrome→hover` ΔL\* 10.5→10.35 故 `.chrome-hover-scope` 维持 light-only（08-07 次生灾害最该复核的一处）、阶梯严格单调、`selected` 仍高于 `hover` 5.0 点；**随后 JC 真机 dogfood 该版本仍要求对调，同日改判落地**——归因被证伪不等于诉求被满足：TN 推翻的是「顺序错了」这个**解释**，而 JC 要的「主对话区是视觉重心」在抬画布之后依然没拿到，抬画布只治了三条真因里的两条；落地方式是**交换而非重新设计**（chrome 取抬画布后的 app 值、app 取原 chrome 值，差值 2.2 只换符号，主区阶梯保持内部步进随 app 回落 2.1，画布净值 19.3→22.4→**20.3**，抬画布未被撤销）；**这是改了一条规则不是调了个数**——「chrome 永远比 app 沉」替换为**「content 走极端、chrome 走中间调」**（light 纸近白故 chrome 更暗、dark 画布近黑故 chrome 更亮，同 macOS 原生侧栏材质），而这条改写反过来解释了旧规则一直在硬撑的事：翻转前的注释必须自己承认 dark 做不到 light 的 ΔL\* 否则侧栏掉进纯黑，**一条需要在某个主题里自我收敛才成立的规则本来就不对称**；连带代价全是事先预警过的那几条并已处理：`.chrome-hover-scope` 从 light-only 变**双主题**（翻转把 `chrome→hover` 从 ΔL\* 10.35 压到 5.49，与 08-07 light 失明同型，dark 分支取值就是**翻转前的全局值**、推导只有一句「sidebar 的地面现在恰好等于翻转前的 app 底」，结果 7.85/5.66 与主区 7.95/5.67 几乎重合）、`--color-selected` 首次进 scope 覆写（否则选中态只剩色相撑，正是 deferred 记着的 light 侧弱点）、chrome(22.4) 与 surface(22.7) 基本持平即 sidebar 与主区卡片同明度带（「浮起来的材质」的必然读法，**有意接受**，注释写明不要挪其中一个去消除碰撞）、正文对比 13.96→**14.67:1**（翻转必然让长文阅读面变暗，Rejected #1 列过的硬伤，已接受，日后发刺解法是墨色那一轮不是把画布再抬回去）；`ComposerActionSlot` 那条「为什么不借用 chrome」的注释**第二条理由被翻转作废但结论更硬**——chrome 十七天内移动两次正是不该借用它的最好证据

### 2026-08-18
- [v0.4.9 发布](./2026-08-18-v0.4.9-release.md) — 聚合 `v0.4.8..HEAD` 4 个提交，**单修复热修**：社区 issue #23 的 Windows 黑窗——Rust 侧 `CREATE_NO_WINDOW` 覆盖是全的，黑窗来自 bridge（无控制台）往下 spawn 控制台子进程时 Windows 新分配的控制台，五个漏网点一次修完（首要是 `workspace_cmd.py` 的 `cmd /c mklink /J`，project workspace 会话准备时执行、与 issue 时机完全吻合；另有 im_reporter 的 CLI 调用、git 探测 ×2、desktop pet、ultraplan daemon）；新增 `runner/process_command.py`（core 同名模块的 Python 镜像），managed-ga 两处走补丁 `0020`（上游漏网、upstream PR 候选）；patch 定级无分歧；**单修复立刻发的理由**：维护侧无 Windows 真机，发布本身就是验证通道（效果只能由报告者装真实构建确认），且未发运补丁会加厚下一版未实测 delta；`0020` 与 `0018`/`0019` 不同**没把未重放债带进发版**（landed 当天即跑 fresh-clone 全栈重放 19/19 + payload 零差异）；已知余量：真机效果待 #23 报告者升级回验，Windows 三音发声待验项与 #16 同题

### 2026-08-14
- [v0.4.8 发布](./2026-08-14-v0.4.8-release.md) — 聚合 `v0.4.7..HEAD` 6 个提交，**至今最小的一次发版且主角不在 GUI**：唯一有分量的是 GA baseline `308153b`→`f06d550`，两处 GUI 改动都是做减法（Channels 自动展开谓词收窄、sidebar 去掉已归档计数），外加一处死配置清理（macOS About 的 `website`/`website_label` 在 macOS 上根本不被消费）；patch 定级无分歧（最大单项是维护不是功能），**值得留痕的是「为什么现在发而不是攒一批」**——baseline 积压有成本（已审计与已发运的 SHA 每差一版，下次要吞的未实测 delta 就更厚，`v0.4.7` 发的还是 `308153b`），而攒批在这里买不到东西（`ga-log-retention` 范围未定、Session Workspace 是暂缓件，且定级规则明写批次总量不影响定级）；用户可感知两条均来自上游：终答不再被无故重生成（`</summary>` 判定加 `len<100` 门槛，此前上游 sys_prompt 要求发 `<summary>` 却把合规终答判成截断，属自伤）、OpenAI 协议模型过载走网络重试而非把 `!!!Error:` 打进正文；**方法教训与 v0.4.7 的「0018 未过全栈重放」同型复发**——我一度用「`pyproject.toml` 零 diff → `GA_DEPS` 未变 → 不必重建 bundle」省掉打包门禁，论证本身没错但**回答的不是门禁在问的问题**（门禁问「新 payload 能不能在打包 Python 下跑起来」，拿上一次构建的 site-packages 去验新 payload 等于验了个和发布件不同的组合），而这道门禁恰是本项目唯一有历史实证会翻车的一条（Telegram 接入时 `python-telegram-bot` 漏进 `GA_DEPS`，正式包里起不来，开发模式 dogfood 验不到），已补从零重建；本机 x86_64 故 `mac-arm64`/`win-x64` 物理不可验（`bundle-python.sh` 靠运行 bundle 里的 python 装依赖），归 release.yml runner；不随本版发运：`ga-log-retention`、Session Workspace、原生 About 收口；**最高风险产物是浏览器扩展**——`background.js` 四处冲突手工解，错误解法会调用已被 `0006` 删除的函数（运行时 `ReferenceError`，Python 编译扫描抓不到），已补 `node --check` 但保不了行为，dogfood 必须真跑一次 Browser Control
- [GA 上游升级 308153b -> f06d550](./2026-08-14-ga-upstream-upgrade-308153b-to-f06d550.md) — 7 提交 11 文件，baseline 引入以来最小的一段；`agent_loop.py`/`agentmain.py`/`pyproject.toml` **零 diff**（故 `GA_DEPS` 未变、`bundle-python.sh` 不重跑，只对既有 bundle 跑烟测），引擎核心只动 `ga.py` + `llmcore.py`，而**真正的工作量在补丁栈 rebase 不在外审**；最值得记的耦合点是 `ga.py:599` 的 `hook(locals())`——上游把 `summary =` 挪进 `if raw:`，于是回合正文只有 tool_use 无文字时（native tool-use 后端常态）这个局部名**根本不存在**，Galley `_on_turn_end` 的 `ctx.get("summary")` 从合成的 `"code_run, args:{...}"` 变成 `None`；无需改代码是因为 `bump_session_after_turn_db` 早有空值守卫（空 summary 跳过该列、保留上一条防止 sidebar 行中途变空），**净效果反而更好**，但那道 Rust 守卫现在是承重的、删它要重验；另两条引擎改动都对 Galley 有利：`</summary>` 不完整判定加了 `len<100` 门槛（上游自己的 sys_prompt 就要求发 `<summary>`，导致合规的终答被当截断反复重生成）、overload 重试从 Claude SSE 推广到三条 OpenAI 路径（managed OpenAI 协议预设不再把 `!!!Error:` 打进正文）；**上游独立收敛到补丁 `0015`**——`e519734`（上游作者，2026-08-13）把页内 `ljq_driver` 徽章改成 click-through + 低透明 + 三态真实状态，正是 `0015`（`9abe74dd`，2026-07-20）删它时列的三条缺陷里的两条，`git log -S` 坐实**是趋同不是任一方采纳**；`0015` 不撤，且这**不是一个需要重审的票**——初版审计误把它当票提给 JC，被当场否掉（「我们之前已经改到页内消失了啊？」），教训值得记：**补丁列出的证据不等于它的理由**，`0015` 的三条抱怨是证据，决定是同一句话的正向半句「状态归 toolbar、不往用户页面注入 UI」，上游把注入的节点改安静碰不到这条；manifest 词条已改写成先说决定、移除条件三项并列并写明「上游打磨徽章不构成触发」；这里真实存在的只是**维护成本**不是产品问题（上游在持续投入 `0015` 删除的那一块，`content.js` 的删除今后每次升级都会撞，rebase 风险已上调 Medium）；rebase 两处真冲突全来自 `e519734`：`0006`/`background.js` 四处中三处是纯加性并存，第四处**只有一个合法解**——上游把 `setStatus('disconnected')` 放进 `isServerAlive()` 的 else 分支而 `0006` 正好删掉这个函数，取上游侧会调用不存在的函数（运行时 `ReferenceError`，Python 编译扫描永远抓不到，故本次补跑 `node --check`）；`0015`/`content.js` 是上游重写了它整块删除的目标，取删除；代价是上游 `status`/`setStatus` 机制在 managed payload 里成为**故意保留的死代码**（删它只会让补丁变大）；`0016`/`0017` 的零上下文 hunk 只发生位置漂移并被 rebase 自动带走，正是不手改 hunk 的理由
- [v0.4.7 发布](./2026-08-13-v0.4.7-release.md) — 聚合 `v0.4.6..HEAD` 35 个提交；两版用户 issue 驱动后**回到自主线**，内容是两批：Discord 渠道（最大单功能）+ **一轮阅读体验与动效打磨马拉松**（13 个提交，性质同 v0.4.3 六轮打磨：markdown 竖向节奏随档位缩放、inline code 暖墨、GFM 复选框、流式挂起代码段/链接闭合、thinking 计时器改 shimmer、run 折叠 grid-rows 扫入、流式光标归位 + 块级软淡入、provider 卡滚动入视野、toast 计时悬停暂停、定时徽章 pop、命令面板语义分组、sidebar 模式切换过渡边缘；另有两次**参考审计零采纳**留痕防重提案）；打磨批**不改定级**——`v0.4.1` 规则判最大单功能不判批次总量，同 v0.4.3；**patch 定级（JC 裁决），而先例指向 minor**——Telegram 作为第三渠道当年是 `v0.3.0` 头条，两条理由压过先例：① `v0.3.0` 同批还塞了 CONC-1..8 并发审计、55 条代码审查、独立产品定位重写，它的 minor 大概率是**批量给的**，而「批次总量」恰恰是 `v0.4.1` 规则明确否掉的定级依据，援引它是弱的 ② 并行监督上下文**只在 Discord 一家生效**（微信/飞书/Telegram 单线语义原样不动），称其为产品线形态升级有抬高之嫌，准确描述是「第四张卡 + 该渠道专属的路由语义」；CLI/Agent API 零改动；`GA_DEPS` 与 `managed-ga/` 双双变动故 **bundled runtime gate 本版强制**；最值得记的是**一个成本论证差点替代了一次验证**——「0018 未过全栈重放」原判断是接受隔离重放、不阻塞，论证走「零重叠」（baseline 没动、0018 独占 `dcapp.py`、前面补丁没一个碰它，故隔离重放数学上等价），论证本身没错但**覆盖不全**：落实那句话时才发现栈里还有个 `0019`，它改的 `frontends/fsapp.py` 正是 `0009`/`0011`/`0012`/`0013` 都改过的文件、靠模糊上下文落位（正是全栈重放要抓的那类），且 `manifest.md` 头部**漏记了它的存在**（还写着「17 patches from here on」，台账行倒是全的）；改口理由不是风险变高（0019 自评 Low 且属实）而是**验证成本远低于预估**——`build-managed-ga.sh` 第 45 行自己写着出路（干净临时 clone at audited baseline），不碰维护者 checkout，且 `git diff` 干净本身就是判据；跑完 18/18 干净应用、payload 零差异，两笔债不是记账是**还清**；教训：**当一个论证被用来替代一次廉价的验证时，先核实它的覆盖范围**——这次漏洞是核实论证前提时撞见的、不是被门禁抓到的，省掉那步 0019 就会带着未验证的债发出去且台账上根本查不到；另一条实测：`bundle-python.sh` 靠**运行** bundle 里的 python 装依赖，故 Intel 机器上 `mac-arm64`/`win-x64` 物理不可验（本地只能过 `mac-x64`，另两个 arch 由 `release.yml` 各自 runner 覆盖、tag 时硬失败），而这道门禁正是本版唯一有历史实证会翻车的一条（`python-telegram-bot` 上次接 Telegram 时漏进 `GA_DEPS`，导致 Telegram 在正式包里起不来，直到本次外审读代码才发现——dogfood 跑开发模式验证不到这层）；不随本版发运：`.scratch/ga-log-retention/`（needs-triage，与 Discord 解耦且范围未定）；上游 `dc_*` vs `discord_*` key 名 bug **裁决不报**（managed 模式 env 注入绕开 mykeys，且对齐的是 dcapp 读侧），该票关闭
- [Discord setup 教程打磨](./2026-08-13-discord-setup-copy-polish.md) — JC 真机配置后的第一手反馈落地：专有名词 inline 链接化（`ConnectionSteps.steps` 宽化 `ReactNode[]` + `stepWithLink` 占位符机制，否决飞书式独立按钮；@BotFather 顺手同办）、setup 文案全量精简 + **症状句删除**（「引路归教程、诊断归错误态」——连接状态机已把 Intent 坑判成带指引的永久错误，§9「症状倒推」条款按此让位）、Step 3 加 **Administrator 权限**引导（JC 提议；采纳的硬理由是缺权限失败静默不可诊断，trade-off 与「共享 Server 场景必须重审」已留痕）、路径选 JC 实测过的 OAuth2 而非未验证的 Installation 页
- [next-suggestion 裸漏全渠道](./2026-08-13-im-suggestion-leak.md) — Discord dogfood 首日翻出 8/4 composer 幽灵文字功能的共享层错位：suggestion mandate 在 `RUNTIME_PROMPT_STATIC` 共享层、消费端却只有 GUI，四个 IM 渠道全在裸漏（reporter 推送同害）。裁决 **C 根修 + A 兜底、B 暂缓**：mandate 拆进 `WORKBENCH_SUGGESTION_PROMPT`、IM spawn 换注入 `compose_im_runtime_prompt`（`prompt_hash` 按原字节拼接不变，证明纯拆分）；0019 补丁给两套 IM 剥离清单 + reporter render 后统一剥（防模型从修复前历史模仿）；「IM 建议按钮化」进 deferred 与根修互斥。经验：**表面专属的提示词 mandate 不进共享层**；Galley 自有标签要同时过 bridge 和 GA 两套剥离清单
- [Discord Channel 落地](./2026-08-13-discord-channel-shipped.md) — 同日从提案走到实现完成（提案→两轮设计→外审 14 条→四票→拆 7 票→实施→全仓验证绿），只差真机 dogfood；第四张 Channels 卡，也是**第一个并行监督上下文渠道**（一个频道/子区 = 一个独立 supervisor 上下文 `galley-im/discord/ch:<id>`，各自 GA agent 实例与历史；CLI/Agent API/`schemaVersion` 零改动——按频道路由用的是既有 message origin 字段）；两个 agent 并行组合拳（一条线扛 reporter dispatcher 改造 + 集成验收，一条线扛补丁/打包/core/GUI），两次接力：04 是验收接力、06 是**额度截断接力**（票面把四个聚合消费者点名列出，接力才没丢东西——票是接力棒不是备忘录）；最值得记的是**集成缝隙案例**——01「managed 重启释放全部活跃频道」× 03「未注册路由目标一律当外人只 mark_seen」组合出重启后漏报（重启前派出的任务在频道重新激活前跑完即被永久吞掉），**两票单看都没错、缝隙在组合处**，修复落 03 侧 `owned_prefixes`（前缀命中但未注册改为挂起、逐 tick 重查、重新激活即补投），教训是多 agent 并行里票与票之间的语义组合是没有主人的地带、需要一次显式集成验收去踩；实现要点：reporter 改进程级单 dispatcher + 路由真相从 session origin 改**消息 origin**（顺手修掉两个现存缺陷：A 渠道建的 session 被 B 续聊推错、GUI 建的被渠道续聊不推；状态键有意保留 per-session `lastReportedMessageId` 而非 PRD 字面的元组键）、dcapp 补丁 0018 的 sentinel 用 `str` 子类实现 `.get()` **正面绕过上游 `run()` 把 `task.get("images")` 写在 `isinstance(task,str)` 之前的缺陷**（走上游自己的 break 分支、不靠异常杀线程）+ 连接状态机分类（token/intent 永久错误立即 error，每个重连周期重建 client）+ LRU 200→12 + 票 4 两件卫生活、per-channel 身份靠 core 的 prompt **模板 seam**（同一函数两次绑定 + 等价断言，杜绝提示词漂移）而非并发改 `os.environ`、Rust 补两道给下一个渠道准备的防漏测试（漏配 pref key / 悄悄落到 WeChat 的锁）；GUI 三处有意偏差（glyph 内联 SVG 非 mask PNG、hint 6 条无 `waiting_scan`、en 保留中文退出命令——`dcapp.py` 词表只认中文）；剩余验收**已随 `v0.4.7` 全部闭合**（真机 dogfood JC 通过无问题、`bundle-python.sh` mac-x64 从零重跑绿、全栈重放 18/18 payload 零差异），上游 `dc_*` vs `discord_*` key 名一票裁决**不报**；PRD 销毁前把九条防重提案裁决迁入本篇（V1 形态推翻 DM-only、否决频道↔session 绑定与共享历史、单 owner 绑定不给 Server 成员开口子、不做常开频道、不加 Application ID 字段、Proxy UI 不做等）
- [Discord Channel 开工](./2026-08-13-discord-channel-kickoff.md) — 暂缓转在办（deferred 台账小节按惯例删除，启动信号 = JC 直接拍板）：PRD 拆成 7 个实施 issues（补丁 → 打包 → reporter dispatcher → runner → Rust core → GUI → 规范/验收）；**先行修复 owner 绑定竞态**（`admit_event` 纯函数按进程代整体拒绝异代事件、`persist_owner` 移入 slots 锁内与灭代序列原子互斥、`unbind_owner` 重排「杀进程→灭代→清 pref→重启」并补 lifecycle 锁、`start_inner` 拆 `start_locked` 防锁重入，三个回归测试；飞书/Telegram 立即受益）
- [Discord 外审四票裁决 + Rule 4 解释入宪](./2026-08-13-discord-review-verdicts.md) — Discord 暂缓 PRD 经 Codex 外审重开四票，JC 当日投毕：输出可见性/激活语义选**纯文档声明 + 沿用上游隐式激活**（否决 owner-only 频道 API 校验捆绑方案，从轻）、owner 配对只认 DM + 按用户限流（**不继承**「10 次全局作废」先例，guild 型渠道通用）、**Rule 4 解释入宪**（GA 引擎自产 `model_responses` 日志属引擎内部状态非 Galley 对话存储，AGENTS.md 已补句，平掉对飞书/Telegram 现状的违宪指控；卫生问题拆 `ga-log-retention` 小项）；外审顺带产出三个与 Discord 解耦的现存 bug（telegram 打包缺口与 toast 漏查当日已修、owner 绑定代际竞态 needs-triage）
- [Command Palette 分组](./2026-08-13-palette-grouping.md) — ⌘K root 列表已长成四种语义（新建 / 会话 / 全文命中 / 操作）默认态 15 行，§8「结果少时 header 是噪音」前提失效，判**重审而非规范赢**：三组走 cmdk `Command.Group` 组头**始终显示**（最近会话 / 在对话内容中 / 操作，沿用原手写头 10px 规格），New chat / New project 裸置顶不进组；未开真机变体（硬理由已足），§8 条款已随裁决改写
...
- [Channels 自动展开谓词收窄](./2026-08-13-channels-auto-expand-predicate.md) — 四张渠道卡的自动展开各说各话（WeChat 只认 attention，Feishu/Telegram/Discord 还把「未接入」「已暂停」和凭证不全算进去），统一成共享谓词 `shouldAutoExpand` = **仅 `waiting_scan` / `expired` / `error`**；`2026-06-11` 的定位（注意力交给 StatusBadge + 自动展开 + primary 按钮）不变，**被推翻的是谓词范围**——「还没配」是绝大多数渠道的**永久静息态而非事件**，拿它触发注意力等于作废注意力：全新用户一进来三张卡同时展开（飞书那张一屏放不下）而系统猜不中他要配哪个，更致命的是**稳定态噪音与使用率成反比**（只用飞书的人，TG 和 Discord 永远展开，代价永久、收益只在头一分钟）；判定原则写进谓词注释——自动展开 = 有事等你动手**且折叠头表达不了**（「未接入」徽章就写着，二维码和错误详情表达不了）；`2026-08-10` 加的 `ready` 双 fetch 门槛**随谓词删除而非回退**（新谓词只读 supervisor state，null 期只会落到不展开的状态，推不出假阳性，最坏是那次明文允许的「折叠→展开」增量出现）；净 -44/+24；不做且留痕：空态引导**不该用展开去当**（要补就补 copy，展开是注意力通道借不得）、维持多开不改手风琴；两项进 deferred（`stopped` 折叠后重启多一次点击的 ghost「启动」补救、展开态跨进出 Settings 记忆）；未开真机变体（硬理由已足，气质票不是决策点）
