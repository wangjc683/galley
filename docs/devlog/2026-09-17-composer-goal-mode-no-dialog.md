...
- **菜单换滚轮（票 10）**：JC 每次打开菜单都觉得该是个 iPhone 闹钟式滚轮。
  我先以「10 分钟粒度是假精度、上限是兜底不是目标、上午刚砍自定义框」反对，
  JC 给出配额窗口场景（Weekly / 5h 配额还剩很多但 135 分钟后 reset，想设
  130 / 140 把配额用掉）——上限对应外部截止时间，反对不成立。三变体真机
  （menu / stepper pill / wheel，10 分钟刻度 10→240 + 无上限）：wheel 在
  macOS 触控板下「丝滑不输 iPhone」，stepper 与 menu 落选。一个显示 bug：
  高亮带绝对定位画在未定位列表之上盖住选中行，列表加 `relative` 修掉。
  封顶 240、保留「推荐」尾标按默认；300（5h 窗口）留作启动信号。
- **三行像菜单**：JC 担心用户打开只见三个完整选项，不会想到滚。诊断：iOS
  滚轮的可滚信号是「边缘行被裁 / 压扁」加「行数够多」，三行完整等距一个都
  没抄。改五行 + 上下 mask 渐隐（渐隐 = 斜面的平面翻译）。被否：三行半露
  半截（上下不对称）、露滚动条（桌面味，反初衷）。
