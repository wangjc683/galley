...
实现上借用 GA 自己的兜底措辞(`ga.py:599`),它本身就是两分支的:无真实
工具时用「直接回答了用户问题」,否则用工具名。新增两个 i18n key
`conversation.stepDirectAnswer` / `stepCalledTools`。

第一版实现只填了最终回答轮,工具轮继续保持纯步号(理由:下方 ToolCallout
已说明发生了什么)。**JC 追问后作废**——那正是把 (B) 的理由破回去:同一
产品又出现了两种形状。查数据定案:全历史 686 行 assistant,`summary`
**一次都没有空过**,GA 总会产出。所以"裸步号"这个形态在本产品里从未存在,
是抑制逻辑自己造出来的,两个分支都必须有词可填。

规模:422 行有正文的轮次里,最终回答轮 echo 6 行,工具轮 echo 2 行。工具轮
罕见但真实,不能靠"反正很少"糊过去。
