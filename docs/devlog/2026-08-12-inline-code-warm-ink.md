...
- ~~**流式悬挂标记补全**~~ → **已做（范围缩小）**，见
  [流式 markdown 补全](./2026-08-12-streaming-markdown-mend.md)。先把抖动
  量化（逐前缀渲染统计「回溯改动」）再决定：实测 16 次 / 位移 230 字符，
  其中行内代码 59%、链接 35%、**强调仅 6% 却占复杂度大头**，故只补代码 span
  和链接目标。顺带推翻了本文早先的一个说法——半截 URL 现在就能点
  （GFM autolink literal），不是补全造成的。
- ~~**间距节奏派生自单一数字**~~ → **已做**，见
