...
- **2026-09-17 补记**：用户消息改为杏沙气泡后 `brand-tint` 的色域比逐行笔触更大（空行、内边距都上色），dark 下的响度问题若存在会被放大；JC 真机裁决时看的仍是 light，本条启动信号不变。
...
- **2026-09-17 补记**：用户消息改回容器形态后，`left-full` 锚的「块右边缘」
  重新成为视觉对象（气泡的绘制边就是布局边），下面「已不是视觉对象」的
  论点失效；只剩 `top-1.5` 锚第一行那一条还成立。
- **方案**：现状 `absolute left-full top-1.5`，两个假设都是 callout 时代
  留下的——`left-full` 锚的「块右边缘」在参差右缘的笔触形态里已不是视觉
  对象，只是布局矩形的边（09-17 起不再成立）；`top-1.5` 锚第一行，而多行消息读完时视线在左下，
