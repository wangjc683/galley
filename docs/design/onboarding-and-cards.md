...
- 字段：`API Key`（label 右侧带 `获取 API Key ↗` 跳转链接，仅当 API 地址
  ...
  `提供商显示名称` 收进 `高级` 折叠。
- 模型列表：Key + 地址齐全后 **800ms 防抖自动拉取**，拉取中「读取模型列表」
  按钮转圈、成功后显示「找到 N 个模型」，让列表有可见的来处；失败静默回到
  空闲，降级为手输。拉到的列表是**模型输入框的候选项**，不是独立区块
  （2026-09-16 起，Settings 新建 provider 同此）：输入框位置不动，尾部亮起
  caret，点开 / ↓ 展开全表，输入即筛选，选中即填入；模型框为空时自动选中
  推荐模型，仍为空时 placeholder 提示「已找到 N 个模型，可从右侧选择」。
  `读取模型列表` 按钮保留为显式刷新路径。
...
`查看 Goal` 这类 CTA，鼠标伸过去的路上消失会点空。下限不会超过
