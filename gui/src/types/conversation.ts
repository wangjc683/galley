...
export interface AgentTurn {
  ...
  turnIndex?: number;
  /**
   * Session-wide persisted turn index used only as a stable UI identity.
   *
   * GA resets `turnIndex` to 1 for every user message, which is correct for
   * display. Live store merging cannot use that display number as identity
   * though: the second user message's "第 1 步" would overwrite the first user
   * message's "第 1 步". SQLite stores a session-wide absolute index; we carry
   * it here so live updates and restored rows merge against the same durable
   * key without changing the user-facing step number.
   */
