export const zhCopy = {
  gitReview: {
    openChanges: "查看工作区改动",
    closeChanges: "关闭工作区改动",
    title: "工作区改动",
    repository: "Git 仓库",
    chooseRepository: "选择仓库",
    changeRepository: "切换仓库",
    chooseHint: "选择一个本地 Git 仓库，查看未提交改动。",
    fromFile: "查看所属仓库改动",
    resizePanel: "调整阅读面板宽度",
    baseline: (head: string) => `相对最近一次提交 · ${head}`,
    baselineCommit: (id: string, subject: string) => `相对 ${id} · ${subject}`,
    baselineShort: (id: string) => `相对 ${id}`,
    unborn: "尚无提交 · 已跟踪文件按新增显示",
    chooseBaseline: "选择比较基线",
    baselineMenu: "比较基线",
    baselineHead: "最近一次提交",
    baselineHeadHint: "工作区相对 HEAD 的未提交改动",
    recentCommits: "较早的提交",
    recentCommitsHint: "选一个提交作为基线，可以看到 Agent 提交之后的累计改动",
    noCommits: "没有可选的提交。",
    loadingCommits: "正在读取提交…",
    invalidBase: "所选提交已不在此仓库中，请重新选择基线。",
    files: (tracked: number, untracked: number) =>
      `${tracked} 个已跟踪文件 · ${untracked} 个未跟踪文件`,
    tracked: "已跟踪文件的改动",
    untracked: "未跟踪文件",
    untrackedHint: "未跟踪列表省略 Git 忽略项、隐藏目录和系统数据目录。",
    selectFile: "选择改动文件",
    status: {
      added: "新增",
      modified: "修改",
    ...
    untrackedContent: "未跟踪文件 · 当前内容",
    netChanges: "已暂存与未暂存的净变化",
    sinceBaseline: "相对所选基线的累计变化（含已提交与未提交）",
    unified: "单列显示",
    split: "双列显示",
    previous: "上一处改动",
    next: "下一处改动",
    ...
    unavailable: "未找到 Git，请安装 Git 后重试。",
    timeout: "读取超时，请稍后刷新或选择较小的仓库。",
    tooLarge: "改动或文件超过预览上限，请使用外部工具查看。",
    encoding: "内容不是可预览的 UTF-8 文本。",
    changed: "仓库状态已变化，请刷新后重新查看。",
    failed: "无法读取仓库，请检查路径及访问权限后重试。",
    binary: "二进制文件不提供文本差异预览。",
    submodule: "子模块发生变化。可选择子模块目录，查看其内部改动。",
    conflicted: "此文件存在未解决的合并冲突，请使用合并工具查看。",
    unsupported: "此文件类型暂不支持预览。",
    unchanged: "此文件已无可显示的差异，请刷新文件列表。",
    metadataOnly: "文件内容未变，仅属性发生变化。",
    renderFailed: "无法显示此差异，请使用外部工具查看。",
  },
  localFiles: {
    resizePreview: "调整文件预览宽度",
    ...
    tooLarge: "文件超过预览上限（文本 2 MiB、图片 10 MiB），无法在此预览。可在文件夹中找到并打开。",
    encoding: "此文件不是可预览的 UTF-8 文本。",
    unsupported: "无法预览或打开此路径。请使用完整的本地文件路径。",
    failed: "文件操作未能完成，请检查文件是否可访问。",
    loading: "正在读取文件…",
    empty: "此文件为空。",
    lines: (count: number) => `${count} 行`,
    tableRows: (rows: number, columns: number) => `${rows} 行 × ${columns} 列`,
    tableTruncated: (shown: number, total: number) =>
      `表格只显示前 ${shown} 行，共 ${total} 行。`,
    tableFallback: "此文件未按规整的分隔格式解析，以纯文本显示。",
    jsonPretty: "已格式化显示；磁盘上的文件为单行 JSON。",
    imageSize: (width: number, height: number) => `${width} × ${height}`,
  },
  common: {
    close: "关闭",
    cancel: "取消",
  ...
  topbar: {
    ...
    goalStageBlocked: "受阻",
    goalStageDone: "已完成",
    goalStageBudgetLimited: "到达上限",
    goalStageStopped: "已停止",
    goalStageFailed: "失败",
    goalElapsed: (minutes: number) => `已用 ${minutes} 分钟`,
    goalElapsedOfCeiling: (minutes: number, ceiling: number) =>
      `${minutes} / ${ceiling} 分钟`,
    goalContinuationCount: (rounds: number) => `已续跑 ${rounds} 轮`,
    goalPausedHint: "已暂停 · 发消息继续",
    goalBlockedHint: "受阻 · 需要你介入",
    ...
    stopGoalConsequence: "停止会中止当前这一轮，Goal 不再续跑",
    extendGoalAtCeiling: (minutes: number) => `再给 ${minutes} 分钟`,
    extendGoal: (minutes: number) => `延长 ${minutes} 分钟`,
  ...
  composer: {
    ...
    pastedImage: "已粘贴图片",
    attachImage: "添加图片",
    attachTooltip: "添加图片或文件",
    referenceFiles: "添加文件…",
    removeImage: "移除图片",
    dropToAttach: "拖放以添加图片或文件",
    dropFilesOnly: "拖放以添加文件（当前运行时不支持图片）",
    goalButton: "Goal",
    goalTooltip: "作为 Goal 运行",
    goalBlockedByRunning: "等当前回复结束后再启动 Goal。",
    goalBlockedByActive: "这个对话已有一个 Goal 在跑，先停掉或等它结束。",
    cancelGoalMode: "取消 Goal 模式",
    startGoal: "启动 Goal",
    startGoalTooltip: "启动 Goal · Enter",
    startGoalWithEnter: "Enter 启动 Goal · Esc 取消",
    goalPlaceholder: "写下要完成的目标…",
    goalContextBadgeTooltip: "这个对话有一个 Goal",
    goalStarting: "启动中…",
    goalCeilingLabel: "时间上限",
    goalCeilingTooltip: "时间上限 · 到上限前 Galley 会一直自己推进",
    goalDurationOption: (minutes: number) => `${minutes} 分钟`,
  ...
  settings: {
    ...
    general: {
      ...
      notifyGoalEndDescription:
        "窗口在后台时，Goal 完成、到达上限、受阻或失败会发送系统通知",
    ...
    runtime: {
      ...
      activeModelDetail: (model: string) => `默认模型 · ${model}`,
      needsModel: "需要先配置模型",
    ...
    models: {
      ...
      reasoningEffortInfo:
        "不设置时不发送该参数，由服务商决定实际强度：OpenAI 系通常为 medium，Claude 由自适应思考按难度分配。Claude 协议上 XHigh 与 Max 是同一档（都映射为最高档）。第三方兼容端点是否支持该参数取决于其实现。",
      reasoningDefault: "由服务商决定",
      reasoningFollowDefaults: (tier: string | null) =>
        tier ? `跟随默认（${tier}）` : "跟随默认（未设置）",
      reasoningProviderDecides: "不设置，由服务商决定",
      followDefaults: "跟随默认",
      overrideCount: (count: number) => `${count} 项覆盖`,
      followDefaultsAll: "全部跟随默认",
      promoteToDefaults: "设为所有模型的默认",
  ...
  conversation: {
    ...
    goalEyebrow: "Goal",
    goalBudgetCeiling: (minutes: number) => `上限 ${minutes} 分钟`,
    goalNoBudget: "无上限",
  ...
  toasts: {
    ...
    goalStartedMessageNoBudget: "Galley 会自己推进这个目标，直到判断完成。",
    goalStartFailed: "Goal 启动失败",
    goalExtended: "Goal 已续上时间",
    goalExtendedMessage: (minutes: number) =>
      `又给了 ${minutes} 分钟，Galley 继续推进。`,
    goalExtendFailed: "续时间失败",
    goalCompleted: "Goal 已完成",
    goalBudgetLimited: "Goal 到达时间上限",
    goalBlocked: "Goal 受阻",
    goalFailed: "Goal 失败",
    viewGoalResult: "查看结果",
    viewGoalDetails: "查看详情",
  ...
  projects: {
    ...
    earlierCount: (count: number) => `${count} 个 30 天前的对话`,
    noEarlier: "暂无早期对话",
    noEarlierEmpty: "30 天前还没有对话。",
  ...
  scheduled: {
    ...
    empty: "还没有定时任务。",
    emptyHint: "定时任务会在设定时间自动开一个新会话执行 Prompt。",
    exampleDigestTitle: "晨间简报",
    exampleDigestPrompt:
      "搜集「你关注的主题」的最新动态和新闻，整理成一份简明的晨间简报。",
    exampleDownloadsTitle: "下载文件夹整理",
    exampleDownloadsPrompt:
      "整理我的下载文件夹：按类型归档到子文件夹，列出超过 30 天没打开过的大文件和安装包，建议哪些可以删除。",
    exampleArchiveTitle: "月度归档",
    ...
    editTask: "编辑定时任务",
    promptLabel: "Prompt",
    promptPlaceholder: "到点后发给新会话的第一条消息…",
    projectLabel: "项目",
    noProject: "不归入项目",
    modelLabel: "模型",
    defaultModel: "默认模型",
    ...
    repeatMonthly: "每月",
    timeLabel: "时间",
    weekdayLabel: (isoDay: number) => "一二三四五六日"[isoDay - 1] ?? "?",
    weekdaysRequired: "每周至少选择一天",
    monthdaysRequired: "每月至少选择一天",
    monthlyClampHint: "不足该日期的月份会在当月最后一天触发",
    repeatSummaryDaily: (time: string) => `每天 ${time}`,
    repeatSummaryWeekly: (days: string, time: string) => `每周${days} ${time}`,
