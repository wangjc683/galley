 ...
 *
 * Display policy (2026-07-03, second pass): the epigraph renders on
 * every empty-state entry, state-bound as designed. A silent-only
 * scarcity gate was tried the same day and reverted after owner
 * dogfood — the state-bound rotation plus per-sitting freeze already
 * prevents wallpaper; chapter epigraphs are a legitimate recurring
 * form. PI §43 additionally serves as the About colophon's quotation
 * (SettingsAbout reads `EPIGRAPHS` directly) — two homes, two
 * registers, accepted.
 */
